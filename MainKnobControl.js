...
function MainKnobKontrol(cursorTrack, transport, cursorClip, cursorDevice) {
    ...
    var deviceBrowser = cursorDevice.createDeviceBrowser(4, 4);
    //var deviceBrowser = cursorTrackDeviceBank.getDevice(0).createDeviceBrowser(4,4);
	
	
	var deviceSession= deviceBrowser.getDeviceSession();
        ...
        switch (mode) {
			...
			default:	
			 if (value === 1) {
				 ...
				 if (shiftReceivers[3].isKeyboardMode()){
					 
					 if (modifiers.isShiftDown()) {
							currentMode.scaleRootDown();
						}else{
							currentMode.scaleDown();
						}
				}else{
                   if (modifiers.isShiftDown()) {
	...
	cursorClip.getLoopLength().addRawValueObserver ( function (value) {
        tempLoopLen = value;
    ...
    browserButton.setCallback(function (value) {
        ...
        if (browsing) {
			...
			browsing = false;
				...
				browsing = true;
				mode = Modes.BROWSER;
			 deviceBrowser.activateSession(deviceSession);
			 if (modifiers.isShiftDown() && bankNrOfDevices>0) {
    ...
    mainPress.setCallback(function (value) {
        ...
        switch (mode) {
            ...
            case Modes.GRID_LEN:
                ...
                break;
			 case Modes.PATTERN_LEN:
				 DefaultLoopLen =  tempLoopLen;
				
                host.showPopupNotification(" CURRENT LENGHT SAVED AS DEFAULT" );
                break;
