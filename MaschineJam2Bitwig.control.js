 ...
 *
 @author mixticninja : 
	...
	You can try this, at your own risks of course, here are my changes so far:
		...
		Added Browse  result column changes with DPAD left/Right and select up/down in list with encoder (from felix version )
		Device add and remove : shift + Browse to insert a device after the selected one
		Control + clear now removes the selected device (selection by control + transport left or right) in control sliders mode.
	 	
		Default  clip duration : encoder click in PATLENGHT Mode ( SHIFT SOLO),
			will stores new ( session wide ) default clip duration.
		
		Playing clip launcher : shift + clip button  to stop  and clip button to just restart the clip.
		
		Added a few scales to keyboard mode.  
		In addition to DPAD, turning Encoder (with all MST, GRP... disabled) in keyboard mode can now switch scales and root note (with shift ).
	
		Corrected drumpad auto color implementation: the pad type color (ie red ) based on name of the pad (ie kick).
		
		Enabled individual drumpad color cutomization by switching tune button (in step or padmode when drum maschine present),
		you'll need for that last to enable the tune button (CC48) in Controller Editor
		
		Show/hide device maximized ( plugin) windows when present new rules : 
			Maximized window is hiden when selecting track with group button. 
			Maximized window is shown when selecting track with SELECT + group button. 
			
		My modifications are GPL Licensed
		That's all folks, i could include your modifications after testing, just fork and let me know...
 *
 */
 ...
 *
 *	TODO 
 *	1.Macros pages switching 
 	2. Follow mode in step mode when patlen > grid size
 *	2b. clear only one sound from clip in step mode
	Next Is it possible ?
 *	3. Open Fx Chain to screen when selecting one
 *  5. note mode with sliders like in maschine dream
 .... Add yours 
...
function init() {
	...
	// shiftReceivers mainKnobControl is later expected to be first in the array
	// do not change order !
	...
	println(" #### Maschine JAM 2 BITWIG BETA Version 0.7 ######");
...
function initCursorTrack(cursorTrack, cursorDevice, padMode, stepMode) {
	...
	function DrumPad(dumStartPos) {

		/**
		 * @param {Channel} channel
		 */
		function registerDrumPadChannel(index, channel,dumStartPos) {
			
			channel.addColorObserver(function (red, green, blue) {
				
				
				if (!padMode.isColorFromName()){
					
					var color = convertColor(red, green, blue);
					var key = index+dumStartPos;	
					padMode.updatePadColor(key,color);
					updateNameMap(key,null);
				}
			});
			
			 channel.addNameObserver( 20, "noname",function (name){
				if (padMode.isColorFromName()){
					var key = index+dumStartPos;
					updateNameMap(key,name);
		...
		for (var i = 0; i < 16; i++) {
			...
			registerDrumPadChannel(i, channel,dumStartPos);
		}

		
	}
	
	function updateNameMap(key,name){
...
function ModeHandler(cliplaunchMode, padMode, stepMode, effectMode, cursorClip, trackHandler, trackViewContainer) {
	...
	var fxButton = controls.createButton(MAIN_BUTTONS.PERFORM);
	 var tuneButton = controls.createButton(MAIN_BUTTONS.TUNE);
	...
	stepMode.setExitHandler(forceExit);
	 ...
	 this.isCliplaunchMode = function () {
        return currentMode===cliplaunchMode;
    };
	
	
	 this.getClipView = function () {
        return cliplaunchMode.getClipView();
    };
	
	 this.isKeyboardMode = function () {
        if ( currentMode===padMode ) {
			return padMode.inNoteView();
		}else if ( currentMode===stepMode ){
			return stepMode.isInPianoMode();
		} else return false;
	...
	tuneButton.setCallback(function (value) {
		
		if (value > 0) {
		
		if (shiftReceivers[3].isKeyboardMode()===false && shiftReceivers[3].isCliplaunchMode()===false){
			// drum pad Mode
			var colorFromName = !padMode.isColorFromName();
			
			padMode.setColorFromName(colorFromName);
			var msg = "auto from kit names";
			if (!colorFromName){
				msg = "from user defined kit colors";
			}
		
			 host.showPopupNotification("DrumPad colors : " + msg);
...
function initLeftButtons() {
	...
	dirPadUp.setCallback(
		function (value) {
			if (value !== 0) {
				if (shiftReceivers[0].isBrowsing()){
					applicationControl.getApplication().arrowKeyUp();
				}else{
	...
	dirPadLeft.setCallback(
		function (value) {
			if (value !== 0) {
				//shiftReceivers[0] is mainKnobControl
				if (shiftReceivers[0].isBrowsing()  ){
					applicationControl.getApplication().arrowKeyLeft();
	...
	dirPadRight.setCallback(
		function (value) {
			...
			if (value !== 0) {
				//shiftReceivers[0] is mainKnobControl
				if (shiftReceivers[0].isBrowsing() ){
					applicationControl.getApplication().arrowKeyRight();
				}else{
	...
	dirPadDown.setCallback(
		function (value) {
			if (value !== 0) {
				if (shiftReceivers[0].isBrowsing()){
					applicationControl.getApplication().arrowKeyDown();
