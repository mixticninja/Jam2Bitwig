...
function nameToColor(name) {
	...
	for (i = 0; i < _color_drums.length; i++) {
		if (_color_drums[i].expr.test(name.toLowerCase())) {
...
function DrumPadView(noteInput, cursorTrack) {
	...
	var stepsEditor = 32;
	var stepsPerView = 32;
	// modifier sur shift tune
	var drumColorFromName = true;
	...
	this.isColorFromName = function() {
		return drumColorFromName;
	}
	
	this.setColorFromName = function(fromName) {
		drumColorFromName = fromName;
		updateJam();
	};
	
	this.updatePadColor = function (padIndex,color) {

		var index = padIndex - baseNote;
		var asig_index = (7 - Math.floor(index / 4)) * 8 + index % 4 + 4;
		println("updatePadColor :" + asig_index);
		colortable[asig_index] =color;
		if (index === selectedNoteIndex) {
			selectedColor = colortable[asig_index];
		}
		if (active) {
		
			controls.buttonMatrix.sendValue(asig_index, colortable[asig_index] + colortableOffset[asig_index], true);	
	...
	function assigneNameToPadColor(padIndex, name) {
		...
		if (name) {
			...
			println("assigneNameToPadColor at index : "+index + "SET BASE");
