...
function ClipLaunchView(trackBank) {
	...
	function registerTrack(track, tindex) {
		...
		slot.addHasContentObserver(function (sindex, hascontent) {
			...
			if (hascontent) playState[_index].createState();
			if(_state.basecolor === 0 && hascontent) {
				playState[_index].basecolor = _state.basecolor;
				sendToJam(_index, playState[_index].color(), true);
				
			}else{
				if (!hascontent){
					playState[_index].delReset();
				sendToJam(_index, 0, true,true);
				}else{
					playState[_index].basecolor = _state.basecolor;
					sendToJam(_index, playState[_index].color(), true);
				}
		...
		slot.addIsPlayingObserver(function (sindex, playing) {
			...
			if (playing)  playState[_index].createState();
		});
	...
	this.creatState = function (index) {
		playState[index].createState();
	}
	...
	//	}
		...
		if (modifiers.isSelectDown()) {
			...
			slots.createEmptyClip(rowt, DefaultLoopLen);
			state.createState();
		} else if (modifiers.isDuplicateDown()) {
			slots.select(rowt);
			slots.duplicateClip(rowt);
			slots.select(rowt+1);
			if(!trackStates[colt].playing) {
				slots.launch(rowt+1);
			}
		} else {
			/** @type {PlayState} */
...
function PlayState() {
	...
	this.delReset = function () {
		pstate=0;
		queued=false;
		hascontent=false;
		playing=false;
		flashing=false;
		flashState= FlashStates.NONE;
		this.exists=false;
		selected=false;
	};
	...
	this.color = function () {
		if (this.basecolor === 0 || ! this.exists) {
			return 0;
		}
