...
function TransportHandler(trackHandler, cursorDevice) {
	...
	this.stateDeviceWindow = function (openit) {
		cursorDevice.isExpanded().set(true);
		cursorDevice.isWindowOpen().set(openit);
    ...
    this.notifyModButton = function (buttonId, isDown) {
        if (buttonId === ENCODERBUTTON.MACRO || buttonId === ENCODERBUTTON.CONTROL) {
				...
				// update flag / clear device dans SliderModeHandler
				if (buttonId === ENCODERBUTTON.CONTROL){
					controls.sliderBank.notifyCtrlDown(isDown);
				}	
            if (isDown) {
